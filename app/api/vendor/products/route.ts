...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!session) {
      return withCORS(NextResponse.json({ message: 'Unauthorized - No session' }, { status: 401 }));
    }

    if (session.user.role !== 'shop_owner') {
      return withCORS(NextResponse.json({ message: 'Unauthorized - Role is not shop_owner' }, { status: 401 }));
    }
...
let shopId = await getShopIdForUser(session.user.id, session.user.shopId ?? undefined);
    ...
    if (!shopId) {
      return withCORS(NextResponse.json({ message: 'Shop ID not found. Please ensure you have completed vendor setup.' }, { status: 401 }));
    }
